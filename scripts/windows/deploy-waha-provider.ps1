...
function Wait-N8nWebhook([string]$Path,[int]$TimeoutSeconds=120) {
  ...
  do {
    $n8n = Get-ComposeContainer 'n8n'
    if ($n8n) {
      $js = "fetch('http://127.0.0.1:5678$Path',{method:'POST',headers:{'content-type':'application/json'},body:'{}'}).then(r=>console.log(r.status)).catch(()=>process.exit(2))"
      $probe = & docker exec $n8n node -e $js 2>$null
...
VALUES('waha-webhook','$hash',now())
ON CONFLICT(name) DO UPDATE SET token_sha256=EXCLUDED.token_sha256,updated_at=now();
"@
Invoke-PgScalar $authSql | Out-Null
$orgSlug = Read-EnvValue 'WAHA_ORGANIZATION_SLUG'
if ([string]::IsNullOrWhiteSpace($orgSlug) -or $orgSlug -match '^CHANGE_ME') {
  $orgSlug = Invoke-PgScalar "SELECT slug FROM organizations ORDER BY (slug='default') DESC, created_at ASC LIMIT 1;"
...
$routeSql = @"
WITH org AS (SELECT id FROM organizations WHERE slug='$orgEsc' LIMIT 1)
INSERT INTO organization_whatsapp_providers(organization_id,provider,session_name,priority,enabled,config,updated_at)
SELECT id,'waha','$sessionEsc',1,true,'{}'::jsonb,now() FROM org
...
if ((Get-Date) -ge $deadline) {
  ...
  if($waha){& docker logs --tail 160 $waha 2>&1|Out-Host}
